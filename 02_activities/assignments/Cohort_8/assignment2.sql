...
*/
DROP VIEW IF EXISTS list_of_products_view;
CREATE VIEW IF NOT EXISTS list_of_products_view AS

SELECT 
product_name || ', ' || product_size|| ' (' || product_qty_type || ')' AS list_of_products
FROM product;

SELECT * from list_of_products_view;
...
SELECT 
coalesce (product_name, '') || ', ' || coalesce (product_size, '') || ' (' || coalesce (product_qty_type, 'unit') || ')' AS list_of_products
FROM product;
...
CREATE TABLE product_units (
    ...
    PRIMARY KEY (product_id, snapshot_timestamp)
);
...
VALUES (
    999,                    -- product_id (use a unique ID)
    'Apple Pie',           -- product_name
    'Large',               -- product_size
    1,                     -- product_category_id (adjust to valid category)
    'unit',                -- product_qty_type
    CURRENT_TIMESTAMP      -- snapshot_timestamp (current time)
);
SELECT * FROM product_units;
