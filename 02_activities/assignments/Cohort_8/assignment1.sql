...
WHERE c.customer_id = 8 AND c.customer_id = 10;
...
LEFT JOIN customer c
ON cp.customer_id = c.customer_id
WHERE c.customer_id BETWEEN 8 AND 10;
...
ORDER BY v.vendor_name, vb.market_date ASC;
...
SELECT v.vendor_id,
	v.vendor_name,
	count(vb.booth_number) AS count_of_rented_booth
FROM vendor v
INNER JOIN vendor_booth_assignments vb
ON v.vendor_id = vb.vendor_id
...
SELECT c.customer_id,
	...
	sum(cp.quantity * cp.cost_to_customer_per_qty) AS total_amt_spent
FROM customer c
INNER JOIN customer_purchases cp
ON c.customer_id = cp.customer_id
GROUP BY c.customer_id, c.customer_first_name, c.customer_last_name
HAVING total_amt_spent > 2000;
